...
from noticias_sin_filtro_server.settings import LAST_VERSION, COMPATIBLE_UPGRADABLE, DEPRECATED_VERSIONS
...
class KillSwtich:
    ...
    def compatibility(version : str, platform: str) -> Compatibility:
        ...
        if version != LAST_VERSION and version not in COMPATIBLE_UPGRADABLE and version not in DEPRECATED_VERSIONS:
            raise ValueError(f"Invalid version: {version}")
        
        # Check if this is a valid platform
        if platform not in Platforms.values():
            raise ValueError(f"Invalid platform: {platform}. Possible platforms: {', '.join(Platforms.values())}")
